...
To use, simply install the package from [npm](http://npm.im/@nuscout/esilnt-config) using `npm` or `yarn`:

```
npm install -D @nuscout/eslint-config
```

## Usage

To set up this config, simply create an `.eslintrc` file in your project's root that extends Scout's configuration:
